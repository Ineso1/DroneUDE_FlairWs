...
void MyLaw::CalculateControl(const Eigen::MatrixXf& stateM, Eigen::MatrixXf& outputMatrix, float dt) {
    ...
    **************************************/
    ...
    Eigen::Vector3f compensation = w_estimation_trans;
    ...
    Fu = u_thrust.norm();
    perturbed_Fu = perturbed_u_thrust.norm();
    ...
    **************************************/
    ...
    Eigen::Vector3f debugRot = RotVec(eq);
        ...
        errorStream << "\nahhhhhhhhhhhhhhh (estimation): (\t" << w_estimation_rot.x() << ",\t" << w_estimation_rot.y() << ",\t" << w_estimation_rot.z() << ")\n";
        errorStream << "\naasfakjsfkjakj (rej): (\t" << translation_rej.x() << ",\t" << translation_rej.y() << ",\t" << translation_rej.z() << ")\n";
